...
function firstApproach() {
    input.forEach(line => {
        let policy, policyRange, policyRangeMin, policyRangeMax, policyChar, password;
        [policy, password] = line.split(": ");
        [policyRange, policyChar] = policy.split(" ");
        [policyRangeMin, policyRangeMax] = policyRange.split("-");

        let charCount = (password.match(new RegExp(policyChar, "g")) || []).length;
        if (charCount >= policyRangeMin && charCount <= policyRangeMax) {
            valid++
        }
    });
    console.log(valid);
